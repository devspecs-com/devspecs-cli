...
func LoadCandidates(db *store.DB, fp store.FilterParams) ([]retrieval.Candidate, error) {
	...
	for _, art := range artifacts {
		...
		sections, _ := db.GetSectionsForArtifact(art.ID)
		var body string
		...
		candidates = append(candidates, ArtifactCandidateWithLinks(art, sources, links, todos, sections, body, extractedJSON))
	}
...
func LoadCandidatesForQuery(db *store.DB, fp store.FilterParams, query string) ([]retrieval.Candidate, error) {
	...
	return retrieval.EnrichCandidatesWithSectionMatches(candidates, query), nil
}
...
func ArtifactCandidate(art store.ArtifactRow, sources []store.SourceRow, todos []store.TodoRow, body, extractedJSON string) retrieval.Candidate {
	return ArtifactCandidateWithLinks(art, sources, nil, todos, nil, body, extractedJSON)
}
...
func retrievalSections(rows []store.SectionRow) []retrieval.IndexedSection {
	if len(rows) == 0 {
		return nil
	}
	out := make([]retrieval.IndexedSection, 0, len(rows))
	for _, row := range rows {
		out = append(out, retrieval.IndexedSection{
			ID:            row.ID,
			...
			Metadata:      sectionMetadata(row.MetadataJSON),
		})
	}
	return out
}

func sectionMetadata(raw string) map[string]string {
	...
	if nested, ok := payload["metadata"].(map[string]any); ok {
