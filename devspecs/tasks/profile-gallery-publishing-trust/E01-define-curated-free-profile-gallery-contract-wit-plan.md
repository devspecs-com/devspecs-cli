# Task profile-gallery-publishing-trust E01 Plan

## Goal
Define curated free profile gallery contract with pinned versions and inspectable files

## Description
Create a bounded planning slice for `Profile gallery and publishing trust`. Use the task index as evidence, then settle the claim, interface, evaluation shape, and known unknowns needed before implementation scope expands.
...
- On-disk paths matched the task but were not indexed: Inspect the warned files or refresh the index before trusting missing context. Evidence: `internal/profiles/profiles_test.go` - on-disk path matched task terms but was not in the indexed candidate set: profile.
...
- [ ] Update `E01-define-curated-free-profile-gallery-contract-wit-result.md` or run `ds task checkpoint`.
