# Task cross-track-route-loops N01 Plan

## Goal
Define route loop artifact model and launch-era manual workflow
...
- [ ] Route loops are defined as explicit authored metadata, not hidden orchestration.
- [ ] The v1.1 route `K01 -> B03 -> H02 -> H03 -> F03 -> K01-1 -> K02 -> K03` is the main fixture.
- [ ] The model explains how promote/improve/block gates affect route progress.
- [ ] The model leaves room for future `ds task list` or route-aware inventory without changing `ds apply next` yet.
...
- [ ] Update `N01-define-route-loop-artifact-model-and-launch-era-result.md` or run `ds task checkpoint`.
