# Task v1-1-command-surface-realignment I04 Plan

## Goal
Harden rich task result completion contract for promote improve iteration loops
...
- Make generated result prompts/templates ask the full completion contract:
  - what slice was attempted
  - which gate was being tested
  - what changed
  - what evidence supports promote versus improve/rework/rollback
  - what remains
  - what the next iteration should be
- Align `ds task prompt`, `ds task checkpoint`, and result templates so agents do not lose the decision-gate loop.
- Preserve the series model: `M00` is the index, `M01` is a slice, `M01-1` is an evidence-driven improvement iteration.
...
- [ ] Result templates force the rich slice completion contract without becoming noisy for small slices.
- [ ] Prompt/checkpoint/result language uses the same promote/improve/rework/rollback/block vocabulary.
...
- [ ] Update `I04-harden-rich-task-result-completion-contract-for-result.md` or run `ds task checkpoint`.
