# Task install-self-update-utilities G03 Plan

## Goal
Document restart shell or IDE after install and upgrade
...
## Dogfood Scenario
ScopeLab hit `ds: command not found` until Cursor restarted. Install docs should explicitly say that shells and IDE terminals may need restart after Homebrew/Scoop/script installs or PATH changes.
...
## Success Criteria
- [ ] README install section includes restart terminal/shell/IDE guidance.
- [ ] Troubleshooting docs include `ds version` as the verification command.
- [ ] Update docs repeat the restart guidance after upgrades when shims/PATH may lag.
- [ ] The guidance stays short enough not to scare first-time users.
...
- [ ] Update `G03-document-restart-shell-or-ide-after-install-and-result.md` or run `ds task checkpoint`.
