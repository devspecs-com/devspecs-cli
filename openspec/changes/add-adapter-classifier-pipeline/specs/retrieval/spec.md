...
#### Scenario: container classifier emits child artifacts
...
- **WHEN** adapter classifiers evaluate the candidate
- **THEN** the OpenSpec container classifier can win with confidence and reasons
...
- **WHEN** the ADR classifier evaluates the document
- **THEN** the classifier emits the ADR model as the winning classifier
...
- **WHEN** no classifier wins with sufficient confidence and separation
...
- **WHEN** classifiers evaluate the candidate
...
- **WHEN** the plan classifier evaluates the file
...
- **GIVEN** a repo defines a local classifier model based on RFC/proposal
...
DevSpecs MUST treat RFC and PRD classifiers as section-pattern families unless real samples justify named subformats.
...
- **WHEN** classifiers evaluate the document
- **THEN** an RFC/proposal classifier can classify it with confidence and reasons
...
- **WHEN** classifiers evaluate the document
- **THEN** the PRD classifier can classify it with confidence and reasons
